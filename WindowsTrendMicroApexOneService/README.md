...
```
Publish-GuestConfigurationPackage -Path './WindowsTrendMicroApexOneService/WindowsTrendMicroApexOneService.zip' `
...
New-GuestConfigurationPolicy `
  ...
  -ContentUri 'https://somestorage.blob.core.windows.net/guestconfiguration/WindowsTrendMicroApexOneService.zip?sv=2020-08-04&st=2022-02-08T17%3A12%3A03Z&se=2025-02-08T17%3A12%3A03Z&sr=b&sp=rl&sig' `
  -DisplayName 'Windows Trend Micro Apex One Service.' `
  -Description 'Compliance check for Windows Trend Micro Apex One. Ensure it is present on VM, Startup set to Automatic and Status is Running' `
  -Path './policies' `
  -Platform 'Windows' `
  -Version 1.0.0 `
  -Verbose
...
Now that you deployed and created a new custom Azure Policy using Guest Configuration, you can deploy the policy to check Windows VMs on Azure and Azure Arc enabled for the WindowsTrendMicroApexOneService. In effect you have a Azure plane using Azure Policy to check for compliance and in more advanced cases using DSC check and change or install software inside the operating system– recall the PSDscResource module and it’s capabilities. PowerShell/PSDscResources (github.com). You have a outer Azure Policy set and managed a cloud scale your inner servers settings.
